    ...
    for (const [gameweek, teams] of gameweekRanks) {
        ...
        sorted.forEach((team, index) => {
            if (index > 0 && team.points === sorted[index - 1].points) {
                // Tie - use previous player's rank
                rankedData.push({
                    ...team,
                    gameweekRank: rankedData[rankedData.length - 1].gameweekRank
                });
            } else {
                // No tie - use current rank (which equals index + 1)
                rankedData.push({
                    ...team,
                    gameweekRank: index + 1
                });
            }
        });
    }
